...
public class GridManager : MonoBehaviour
{
    ...
    private void EnsureGameManagerExists()
    {
        ...
        {
            var existingManager = FindAnyObjectByType<GameManager>();
            if (existingManager != null)
            {
                Debug.Log("Found existing GameManager in scene");
                return;
            }
            else
            {
                Debug.Log("Creating a new GameManager GameObject");
    ...
    void GenerateGrid()
    {
        if (GameManager.Instance == null)
        {
            Debug.LogError("GameManager.Instance is still null after attempting to create it. Check script execution order.");
            return;
        }

        var grid = new Cell[_width, _height];
        ...
        GameManager.Instance.SetupGrid(grid);
    }
}
